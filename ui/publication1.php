...
<div id="container">
	...
	<div id="content">
		...
		if (isset($_GET['action'])) {
			$var=$_GET['action'];
			$sql = "SELECT * FROM books WHERE Publication like '%$var%'";
		    $result = mysqli_query($connect,$sql);
			if($result!=false){
			if (mysqli_num_rows ($result) >0 )
			{
				echo '<br><br><br><br><h2 style="text-transform: uppercase;text-align:center;color:#1C478E">Books found published by "'.$var.'"<br></h2>';
				...
				while($row=mysqli_fetch_array($result))
				{
					if ($r%2==0)
						echo '<tr>';
					echo '<td style="padding:20px"><a href="page23.php?action='.$row[1].'"><img align=top width=200px class="abc" src="'.$row[9].'">';
			...
			{
				echo '<br><br><br><h2 style="color:#1C478E;text-transform:uppercase;text-align:center">No books published by '.$var.' available </h2>';
			}
			}
		}
		?>
