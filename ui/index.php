...
function validate1()
	{
		var name=document.forms["custrgr"]["name"].value;
		var phone=document.forms["custrgr"]["phone"].value;
		var city=document.forms["custrgr"]["city"].value;
		var state=document.forms["custrgr"]["state"].value;
		var phone=document.forms["custrgr"]["pin"].value;
		
		if(!/^[A-Za-z]*$/g.test(name))
		{
			alert("Name field cannot contain digits or special characters");
			return false;
		}
		else if(!/^[1-9]*$/g.test(phone))
		{
			window.alert("Phone Number field can contain only digits");
			return false;
		}
		else if(!/^[A-Za-z]*$/g.test(city))
		{
			window.alert("City field can only contain letters");
			return false;
		}	
		else if(!/^[A-Za-z]*$/g.test(state))
		{
			window.alert("State field can only contain letters");
			return false;
		}
		else if(!/^[1-9]*$/g.test(pin))
		{
			window.alert("Pin Code field can only contain digits");
			return false;
		}
		else
			return true;
	...
	function validate2()
	{
		var name=document.forms["shopkprrgr"]["ownername"].value;
		var phone=document.forms["shopkprrgr"]["phone"].value;
		var city=document.forms["shopkprrgr"]["city"].value;
		var state=document.forms["shopkprrgr"]["state"].value;
		var phone=document.forms["shopkprrgr"]["pin"].value;
...
<!--for retrieving books data on index.php-->
		<?php
			$sql_books= "SELECT * from books";
			$result_users=mysqli_query($connect,$sql_books);
			...
			while($row=mysqli_fetch_array($result_users))
			{
				...
				if ($r%2==0)
					echo '<tr>';
				echo '<td style="padding:20px"><a class ="a" href="page21.php?action='.$row[1].'"><img align=top width=200px class="abc" src="'.$row[9].'">';
...
<div id="regbtn" class="modal">
  ...
  <div class="modal-content1 animate">
	...
	<form id="shopkprrgr" action="register_insert_shp.php" method="post" onSubmit="return validate2">
	...
	<form id="custrgr" action="register_insert.php" method="post" onSubmit="validate1()">
